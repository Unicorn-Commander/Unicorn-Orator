...
const voiceData = {
  // ── Pocket TTS voices (primary engine, Kyutai Labs) ──────────
  "jean": {
    name: "Jean",
    quality: 5,
  ...
  "fantine": {
    name: "Fantine",
    quality: 5,
    description: "Warm, emotional female voice with gentle tone",
    gender: "female",
    accent: "Pocket TTS",
    engine: "pocket-tts"
  },
  ...
  "azelma": {
    name: "Azelma",
    quality: 4,
    description: "Distinctive female voice with unique character",
    gender: "female",
    accent: "Pocket TTS",
    engine: "pocket-tts"
  },

  // ── Kokoro ONNX voices (fallback engine) ─────────────────────
  "af": {
    name: "AF - Default", 
    quality: 4, 
...
function groupVoices() {
  ...
  Object.entries(voiceData).forEach(([id, data]) => {
    if (data.engine === 'pocket-tts') {
      groups['Pocket TTS (Primary)'].push({ id, ...data });
    } else {
      const category = `Kokoro - ${data.accent} ${data.gender.charAt(0).toUpperCase() + data.gender.slice(1)}`;
      if (groups[category]) {
